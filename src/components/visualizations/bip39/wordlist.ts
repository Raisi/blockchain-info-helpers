...
export async function loadWordlist(): Promise<string[]> {
  ...
  // Fallback to partial list
  cachedWordlist = PARTIAL_WORDLIST;
  return PARTIAL_WORDLIST;
}

export function getWord(wordlist: string[], idx: number): string {
  return idx < wordlist.length ? wordlist[idx] : `word_${idx}`;
}
